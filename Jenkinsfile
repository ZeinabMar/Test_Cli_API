            ...
            steps {
                script {
                    ...
                    sshagent (credentials: ['olt_qa_qms_account']) {
                        sh "ssh -o StrictHostKeyChecking=no -p 5555 ${USERNAME}@192.168.1.47 qa -n ${TEST_RUN_NAME} -p test_sp5100_cli -r ${revision_type},${revision_value} -s \\\'--stepwise\\\' \\\'--cache-clear\\\' \"${test_suit_path}\" -vv || true"
                    }
                }
            }
        }
